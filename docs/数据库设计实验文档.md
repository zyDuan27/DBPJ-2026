...
本数据库服务于校园活动报名系统，覆盖用户登录、活动发布与审核、场地与分类字典、活动报名、候补递补、现场签到、活动评价、信用分流水和基础统计等功能。优化后的设计目标如下：

- 用外键、唯一约束、检查约束和触发器共同保证核心业务一致性。
- 保留必要的性能型冗余字段 `Activity.current_enrollment`，但由触发器统一维护，避免应用层遗漏。
- 消除不必要的信息冗余：活动表不重复保存校区名、场馆名、分类名，报名表不重复保存学生姓名和手机号，均通过多表查询获得。
- 为活动列表、场地冲突检测、报名名单、候补队列、反馈看板和信用统计建立针对性组合索引。
...
- `Activity.current_enrollment` 是性能型冗余字段，只统计 `ENROLLED` 与 `CHECKED_IN` 状态。该字段避免活动列表频繁聚合报名表，由 `Registration` 触发器统一维护。
...
CREATE TABLE Activity (
    ...
    admin_id INT,
    CONSTRAINT fk_act_venue FOREIGN KEY (venue_id) REFERENCES Venue(venue_id) ON UPDATE CASCADE ON DELETE RESTRICT,
    CONSTRAINT fk_act_category FOREIGN KEY (category_id) REFERENCES Category(category_id) ON UPDATE CASCADE ON DELETE RESTRICT,
...
- 字典被业务数据引用时默认 `ON DELETE RESTRICT`，防止误删校区、场地、分类、组织者。
...
- `trg_registration_ai_enrollment`、`trg_registration_au_enrollment`、`trg_registration_ad_enrollment`：根据报名状态插入、更新、删除自动维护 `Activity.current_enrollment`。
...
系统没有在活动表中冗余校区名、场地名、房间号和分类名，活动列表通过如下多表连接查询：
...
初始化脚本包含学生、组织者、管理员、校区、场地、分类、活动、报名、评价和信用流水，可用于快速验证主要业务闭环。
...
INSERT INTO Activity(title, start_time, end_time, enroll_deadline, capacity_limit,
...
测试数据执行后，活动 1 的 `current_enrollment` 应由触发器自动维护为 `2`，其中一名学生已签到、一名学生正选报名；活动 2 有一名候补学生，不占用名额。
...
```sql
-- 检查 current_enrollment 是否与报名表一致
SELECT a.activity_id, a.current_enrollment,
       SUM(CASE WHEN r.status IN ('ENROLLED', 'CHECKED_IN') THEN 1 ELSE 0 END) AS real_enrollment
FROM Activity a
LEFT JOIN Registration r ON a.activity_id = r.activity_id
GROUP BY a.activity_id, a.current_enrollment;

-- 检查是否存在同场地时间冲突
SELECT a1.activity_id AS activity_one, a2.activity_id AS activity_two, a1.venue_id
FROM Activity a1
JOIN Activity a2 ON a1.venue_id = a2.venue_id
                AND a1.activity_id < a2.activity_id
                AND a1.start_time < a2.end_time
                AND a1.end_time > a2.start_time
WHERE a1.status IN ('PENDING_REVIEW', 'PUBLISHED', 'ONGOING')
  AND a2.status IN ('PENDING_REVIEW', 'PUBLISHED', 'ONGOING');

-- 查看学生信用分
SELECT u.user_id, u.username, 100 + COALESCE(SUM(c.change_value), 0) AS credit_score
FROM User u
