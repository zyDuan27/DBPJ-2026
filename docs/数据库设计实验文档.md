...
自然语言查询采用“LLM 理解 + 受控逻辑视图 SQL 草稿 + 后端安全编译”的混合方案。学生和组织者默认走 `CONTROLLED_SQL`，模型只能引用后端声明的逻辑视图和业务字段；管理员可走 `ADMIN_SQL` 只读草稿模式。DSL 与规则解析器仍保留为兜底路径。

实现要点：

- `LlmQueryPlanner` 调用 OpenAI-compatible Chat Completions API，要求模型输出固定 JSON，可选 `CONTROLLED_SQL`、`ADMIN_SQL` 或 DSL。
- `ControlledSqlCompiler` 校验普通用户 SQL 草稿，只允许单条 `SELECT`、逻辑视图白名单字段和 `LIMIT <= 50`，并拒绝真实表、敏感字段、多语句和写操作。
- `AdminSqlValidator` 校验管理员 SQL 草稿，只允许业务表白名单内的只读查询，并过滤 password、token、secret、hash、phone 等敏感字段。
- `QueryPlanValidator` 与 `QuerySqlBuilder` 保留为 DSL 兜底路径，校验意图、字段别名、操作符、枚举值和分页上限。
- 权限条件由后端根据当前角色强制注入。学生逻辑视图只暴露公开活动、自己的报名、自己的信用和通知；组织者逻辑视图只暴露自己活动相关数据；管理员可查全站业务数据。
